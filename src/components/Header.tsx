  ...
  return (
    <motion.header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
        scrolled
          ...
          : "bg-gradient-to-br from-[#0A1A3F] via-[#0E225A] to-[#05102A] dark:bg-[#0A1A3F]"
      } dark:bg-[#0A1A3F] bg-white/90 shadow-md`}
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 lg:px-8 py-4">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {links.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-white font-medium text-sm uppercase tracking-wide transition"
            >
              {link.label}
            </a>
          ))}
        ...
        {/* --- Bouton Mobile --- */}
        <button
          className="lg:hidden text-gray-800 dark:text-white hover:text-blue-500 transition"
          onClick={() => setMenuOpen(!menuOpen)}
        >
          {menuOpen ? (
            <X size={28} />
          ) : (
            /* Icône personnalisée avec deux traits */
            <div className="flex flex-col justify-between w-6 h-3">
              <span className="block h-0.5 w-full bg-current rounded"></span>
              <span className="block h-0.5 w-full bg-current rounded"></span>
            </div>
          )}
        </button>
